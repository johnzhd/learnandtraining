...
static void finish_attribute_value(GumboParser* parser) {
  ...
  GumboAttribute* attr =
      (GumboAttribute*)tag_state->_attributes.data[tag_state->_attributes.length - 1];
...
void gumbo_tokenizer_state_init(
  ...
  GumboTokenizerState* tokenizer =
      (GumboTokenizerState*)gumbo_parser_allocate(parser, sizeof(GumboTokenizerState));
...
static StateResult handle_data_state(
  ...
  switch (c) {
    ...
    case '\0':
      ...
      return RETURN_ERROR;
    default:
      return emit_current_char(parser, output)?RETURN_SUCCESS:RETURN_ERROR;
  }
}
...
static StateResult handle_end_tag_open_state(
  ...
  switch (c) {
    ...
    case -1:
      ...
      gumbo_tokenizer_set_state(parser, GUMBO_LEX_DATA);
      return emit_temporary_buffer(parser, output)?RETURN_SUCCESS:RETURN_ERROR;
    default:
      if (is_alpha(c)) {
        gumbo_tokenizer_set_state(parser, GUMBO_LEX_TAG_NAME);
...
static StateResult handle_rcdata_end_tag_open_state(
  ...
  return true?RETURN_SUCCESS:RETURN_ERROR;
}
